...
{
    if ([self.imagePickerController.delegate respondsToSelector:@selector(qb_imagePickerController:didCancelAssets:)]) {
		if ([self.imagePickerController.selectedAssetURLs count]) {
			[self fetchAssetsFromSelectedAssetURLsWithCompletion:^(NSArray *assets) {
				[self.imagePickerController.delegate qb_imagePickerController:self.imagePickerController didCancelAssets:assets];
			}];
		} else {
			[self.imagePickerController.delegate qb_imagePickerController:self.imagePickerController didCancelAssets:nil];
		}
    }
}
